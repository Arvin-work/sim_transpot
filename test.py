...
class Intersection:
    ...
    def plot_metrics(self):
        ...
        for direction, lengths in metrics["queue_lengths"].items():
            if lengths:  # 确保有数据
                axs[0, 0].plot(lengths, label=direction)
        
        if axs[0, 0].lines:  # 如果有线条才设置标题等
            axs[0, 0].set_title("各方向队列长度变化")
            axs[0, 0].set_xlabel("时间步")
            axs[0, 0].set_ylabel("队列长度")
            axs[0, 0].legend()
        ...
        # 只有当有数据时才绘制条形图
        if any(wait_times):
            axs[0, 1].bar(directions, wait_times, color='skyblue')
            axs[0, 1].set_title("各方向平均队列长度")
            axs[0, 1].set_ylabel("平均队列长度")
        ...
        if throughput_data:  # 确保有数据
            sns.countplot(x=throughput_data, ax=axs[1, 0], 
                         order=directions)
            axs[1, 0].set_title("各方向通行量分布")
            axs[1, 0].set_xlabel("方向")
            axs[1, 0].set_ylabel("通行量")
        else:
            axs[1, 0].text(0.5, 0.5, "无通行量数据", 
                          ha='center', va='center', 
        ...
        if travel_times:  # 确保有数据
            axs[1, 1].hist(travel_times, bins=20, color='green', alpha=0.7)
            axs[1, 1].set_title("车辆行程时间分布")
            axs[1, 1].set_xlabel("行程时间 (秒)")
            axs[1, 1].set_ylabel("车辆数")
            axs[1, 1].grid(True, linestyle='--', alpha=0.7)
        else:
            axs[1, 1].text(0.5, 0.5, "无行程时间数据", 
                          ha='center', va='center', 
